...
Every database command is launched by a Python supervisor that opens the selected state root from the filesystem root one component at a time without following symlinks, retains the full ancestor descriptor identity chain, and holds an exclusive lock on the stable state descriptor until the command finishes.
...
The database replace is the publication linearization point.
